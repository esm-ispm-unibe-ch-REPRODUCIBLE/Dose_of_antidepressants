    ...
    tryCatch({#start tryCatch to avoid stopping with stupid errors
      ...
      with(predict(doseresRR, newdata,xref, exp = TRUE), {
        plot(get("rcs(Dose_delivered_mean, knots)Dose_delivered_mean"),pred, type = "l",
             xlim = c(0, xmax), ylim = c(.5, 4),xlab="Actual mean dose",ylab="RR",main=c("Splines",text),col="deepskyblue",lwd=2)
        matlines(get("rcs(Dose_delivered_mean, knots)Dose_delivered_mean"),cbind(ci.ub,ci.lb),col="deepskyblue",lty="dashed",lwd=0.5)})
...
{#iterate in drugs
  ...
  if(max(mymoredata$logRRdropAE,na.rm = T)==0 | length(unique(mymoredata$Study_No))<2){cat(paste("not enough dropout data"),"\n")}
    ...
    tryCatch({#start tryCatch to avoid stopping with stupid errors
      ...
      with(predict(doseresRR, newdata,xref, exp = TRUE), {
        plot(get("rcs(Dose_delivered_mean, knots)Dose_delivered_mean"),pred, type = "l",
             xlim = c(0, xmax), ylim = c(.5, 4),xlab="Actual mean dose",ylab="RR dropout AE",main=c("Splines",text),col="deeppink",lwd=2)
        matlines(get("rcs(Dose_delivered_mean, knots)Dose_delivered_mean"),cbind(ci.ub,ci.lb),col="deeppink",lty="dashed",lwd=0.5)})
      with(mymoredata,rug(Dose_delivered_mean, quiet = TRUE))},error=function(e){cat("ERROR in Spline",":",conditionMessage(e), "\n")})
...
{#iterate in drugs
  ...
  if(max(mymoredata$logRRdrop,na.rm = T)==0 | length(unique(mymoredata$Study_No))<2){cat(paste("not enough dropout data"),"\n")}
    ...
    tryCatch({#start tryCatch to avoid stopping with stupid errors
      ...
      with(predict(doseresRR, newdata,xref, exp = TRUE), {
        plot(get("rcs(Dose_delivered_mean, knots)Dose_delivered_mean"),pred, type = "l",
             xlim = c(0, xmax), ylim = c(.5, 4),xlab="Actual mean dose",ylab="RR dropout",main=c("Splines",text),col="purple",lwd=2)
        matlines(get("rcs(Dose_delivered_mean, knots)Dose_delivered_mean"),cbind(ci.ub,ci.lb),col="purple",lty="dashed",lwd=0.5)})
      with(mymoredata,rug(Dose_delivered_mean, quiet = TRUE))},error=function(e){cat("ERROR in Spline",":",conditionMessage(e), "\n")})
